...
const OrderBook = ({ marketId = 'C3YPL3kYCSYKsmHcHrPWx1632GUXGqi2yMXJbfeCc57q' }) => {
  ...
  const shouldFlash = (currentOrder: Order, previousOrder: Order | undefined, side: 'bid' | 'ask') => {
    if (!previousOrder || currentOrder.sizePercent === previousOrder.sizePercent) {
      return '';
    }
  ...
  useEffect(() => {
    const fetchOrderBookData = async () => {
      try {
        const response = await fetch(`https://alpha.arcana.markets/api/openbookv2/markets/${marketId}/orders`);
        if (!response.ok) throw new Error('Network response was not ok');
        let data: OrderBookData = await response.json();
        data.market.bidOrders.sort((a, b) => b.price - a.price);
        data.market.askOrders.sort((a, b) => a.price - b.price);
    ...
    return () => clearInterval(interval);
      ...
      const totalBidSize = bidOrders.reduce((acc, order) => acc + order.size, 0);
      const totalAskSize = askOrders.reduce((acc, order) => acc + order.size, 0);
        ...
        <div className='w-full flex min-w-[750px] flex-col gap-[2px] max-h-[250px] sm:max-h-[350px] md:max-h-[480px] overflow-auto hideScrollBar'>
          {filteredRowsData.map((item, index) => (
            <div key={index} className='w-full grid grid-cols-[1fr,4fr,1fr] min-h-[35px] justify-center items-center relative'>
              {/* Size percent bars */}
              <div
                style={{ width: `${item.bid.sizePercent}%`, backgroundColor: '#4EDF87', height: '100%', position: 'absolute', right: '50%' }}
                className='size-percent-bar'
              ></div>
              <div
                style={{ width: `${item.ask.sizePercent}%`, backgroundColor: '#F9564F', height: '100%', position: 'absolute', left: '50%' }}
                className='size-percent-bar'
              ></div>
                {/* bid owner */}
