...
import { OpenBookTradeEvent } from '@/utils/types';
  ...
  const TradeHistory =  ({ marketId = 'C3YPL3kYCSYKsmHcHrPWx1632GUXGqi2yMXJbfeCc57q' }) => {
    const [tradeHistory, setTradeHistory] = useState<OpenBookTradeEvent[]>([]);
  
  useEffect(() => {
    fetch(`https://alpha.arcana.markets/api/openbookv2/markets/${marketId}/trades`)
  ...
  const renderTradeRow = (item: OpenBookTradeEvent | null, index: number) => {
    ...
    const rowBackground = index % 2 === 0 ? 'bg-background-800 dark:bg-[#012a36]' : 'bg-background-900 dark:bg-[#09303c]';
        ...
        {item ? (
          <>
            ...
            <div className='w-full flex justify-start items-center'>
              ...
              <a href={`https://solscan.io/account/${item.takerOwner}`} target="_blank" rel="noopener noreferrer">
            ...
            <div className='w-full flex justify-end items-center'>
              ...
              <a href={`https://solscan.io/account/${item.makerOwner}`} target="_blank" rel="noopener noreferrer">
                <button className='p-1'>
                  <Icons.share />
                </button>
              </a>
  ...
  const historyTableHeader = ['trade', 'Timestamp', 'Price', 'Size', 'Taker', 'Maker'];
    ...
    <div className='w-full flex flex-col gap-4 items-start'>
      ...
      <div className='w-full flex flex-col overflow-auto hideScrollBar bg-background-900 dark:bg-[#09303c] border-[1px] borderColor rounded-[16px]'>
        ...
        <div className='w-full min-w-[980px] max-h-[200px] sm:max-h-[250px] overflow-auto hideScrollBar'>
          ...
          {Array.from({ length: Math.max(0, 6 - tradeHistory.length) }, (_, index) => renderTradeRow(null, tradeHistory.length + index))}
        </div>
      </div>
    </div>
  );
