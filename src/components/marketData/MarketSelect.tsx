...
const MarketSelect = () => {
  const { setMarketData, setTradeHistory, setOrderBook } = useMarketStore((state) => state);
  const [marketId, setMarketId] = useState("C3YPL3kYCSYKsmHcHrPWx1632GUXGqi2yMXJbfeCc57q");
  ...
  const handleChange = (market: React.SetStateAction<null>) => {
    setSelected(market);
  ...
  const renderMarketOption = (marketData: any) => {
    ...
    return (
      <div className='flex items-center'>
        <Image src={baseTokenLogo} alt={baseToken} width={18} height={18} className='object-fill z-10' />
        <Image src={quoteTokenLogo} alt={quoteToken} width={18} height={18} className='object-fill -ml-2 z-0' />
            ...
            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-background-100 cardShadowBor py-1 text-left shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {markets.map((market) => (
                <Listbox.Option
                  ...
                  value={market}
                  className={({ active }) => `relative cursor-default select-none py-2 pl-4 pr-4 ${active ? 'bg-primary-100 text-white' : 'text-white'}`}
                >
