...
const MarketInformation = ({ marketId = 'C3YPL3kYCSYKsmHcHrPWx1632GUXGqi2yMXJbfeCc57q' }) => {
  const [marketDetails, setMarketDetails] = useState<MarketData | null>(null);
  ...
  useEffect(() => {
    if (marketDetails?.market.name === 'SOL-USDC') {
      getToken('solana')
        .then(data => {
  ...
  const [baseToken, quoteToken] = marketDetails?.market.name ? marketDetails.market.name.split(/[-\/]/) : ['', ''];

  const baseTokenLink = marketDetails?.market?.baseMint ?? 'defaultBaseMint';
  const quoteTokenLink = marketDetails?.market?.quoteMint ?? 'defaultQuoteMint';
  ...
  const details = [
    { name: 'Market ID', value: marketDetails.market.marketId },
    { name: 'Base Mint', value: marketDetails.market.baseMint },
    { name: 'Quote Mint', value: marketDetails.market.quoteMint },
    { name: 'Bids', value: marketDetails.market.bids },
    { name: 'Asks', value: marketDetails.market.asks },
    { name: 'Event Queue', value: marketDetails.market.eventHeap },
  ];
  ...
  const totalVolume = marketDetails ?
    (marketDetails.market.takerVolume + marketDetails.market.makerVolume) / Math.pow(10, marketDetails.market.quoteDecimals) :
    0;
      ...
      <div className='w-full bg-transparent flex flex-col gap-4 sm:gap-8 px-4 sm:px-8 py-4 border-b-[1px] borderColor'>
        <div className='flex sm:gap-0 gap-3 justify-between items-start'>
          ...
          <div className='flex flex-col gap-2 sm:gap-3 items-start'>
            ...
            <p className={`text-[18px] sm:text-[24px] font-medium ${getPriceChangeClassName()}`}>$ {marketDetails?.midpoint.toFixed(4)}</p>
          </div>
          {/* 24h Low */}
          <div className='flex flex-col gap-2 sm:gap-3 items-start'>
            <p className='text-[12px] sm:text-[14px] text-white/50 font-medium'>
            ...
            <p className={`text-[18px] sm:text-[24px] font-medium ${getPriceChangeClassName()}`}>{priceChange?.toFixed(2) ?? '0.00'}%</p>
          </div>
          <div className='flex flex-col gap-2 sm:gap-3 items-start'>
            <p className='text-[12px] sm:text-[14px] text-white/50 font-medium'>
              24h USD Volume
          ...
          <div className='w-full flex flex-wrap justify-start items-center gap-3 sm:gap-4'>
            ...
            <div className='flex flex-col items-center gap-2'>
              ...
              <div className='flex items-center bg-background-800 dark:bg-[#012A36] py-[6px] px-[8px] rounded-[8px] gap-2'>
                ...
                <Link href={`https://solscan.io/account/${baseTokenLink}`} target="_blank" rel="noopener noreferrer">
                <button className='flex justify-center items-center'> {/* Flex container with center alignment */}
            ...
            <div className='flex flex-col items-center gap-2'>
              ...
              <div className='flex items-center bg-background-800 dark:bg-[#012A36] py-[6px] px-[8px] rounded-[8px] gap-2'>
                ...
                <Link href={`https://solscan.io/account/${quoteTokenLink}`} target="_blank" rel="noopener noreferrer">
                <button className='flex justify-center items-center'> {/* Flex container with center alignment */}
                  <Icons.share ClassName='w-[20px] h-[20px]' />
                </button>
              </Link>
