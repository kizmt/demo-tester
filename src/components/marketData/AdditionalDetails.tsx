...
const AdditionalDetails = () => {
  const { marketId, fetchMarketData, setMarketId, marketData } = useMarketStore();


  useEffect(() => {
    fetchMarketData(); // Replace with your default market ID
  }, [setMarketId, marketId, fetchMarketData]);

  // Split the market name to get base and quote tokens
  // This regex will split the string on either a slash or a hyphen
  const [baseToken, quoteToken] = marketData?.name ? marketData.name.split(/[-\/]/) : ['', ''];
        ...
        <div className='flex flex-col items-start gap-4 px-4 sm:px-8 py-4 border-b-[1px] borderColor'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <div className='flex gap-2 justify-center items-center'>
              ...
              <p className='text-foreground-100 dark:opacity-100 opacity-80 dark:text-white text-[18px] sm:text-[24px] font-semibold'>
                ...
                {((marketData?.baseDepositTotal ?? 0) / Math.pow(10, marketData?.baseDecimals ?? 0)).toLocaleString()}
              </p>
            </div>
          </div>
          {/* Quote deposits */}
          <div className='flex flex-col gap-2'>
            ...
            <div className='flex gap-2 justify-center items-center'>
              ...
              <p className='text-foreground-100 dark:opacity-100 opacity-80 dark:text-white text-[18px] sm:text-[24px] font-semibold'>
                {/* Dynamic value from API */}
                {((marketData?.quoteDepositTotal ?? 0) / Math.pow(10, marketData?.quoteDecimals ?? 0)).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
